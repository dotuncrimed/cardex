...
import { rankOf, suitOf, suitSymbol, isRedSuit, sortCards } from "./cards.js";
...
function renderOpponentClusters() {
  ...
  positions.forEach((p) => {
    const el = $("#cluster-" + p);
    if (!el) return;
    if (!show) { el.classList.add("hidden"); el.innerHTML = ""; delete el.dataset.filled; }
  });
  if (!show) return;
  const myPlayer = currentUserPlayerObject();
  const mySeat = myPlayer ? myPlayer.seat : 0;
...
function selectRow(row) {
  state.targetRow = row;
  document.querySelectorAll(".pg-row-tab").forEach((tab) => tab.classList.toggle("active", tab.dataset.row === row));
}
...
function renderRoomInfo() {
  ...
  if (timerElement) {
    if (state.room.phaseEndsAt) {
      const secondsLeft = Math.max(0, Math.ceil((state.room.phaseEndsAt - Date.now()) / 1000));
...
function renderLobbySection() {
  ...
  if (status === "lobby" && lobbySection) lobbySection.classList.remove("hidden");
  ...
  if (startButton) startButton.classList.toggle("hidden", !isHost() || !["lobby", "round_end"].includes(status));
  ...
  if (joinGameLobbyBtn) {
    ...
    joinGameLobbyBtn.classList.toggle("hidden", !(canTakeSeat && hasMoney));
  }
  renderPlayerList();
}
...
function rowScoreFor(results, uid, rowKey) {
  ...
  return list.reduce((sum, d) => sum + ((d.rows && d.rows[rowKey]) || 0), 0);
}
...
function skipReveal() {
  ...
  state.room.players.forEach((pl) => {
    ...
    if (spec) specialPlayers.push(`${pl.displayName} — ${spec.name}`);
  });
...
function disableArrangeControls(disabled) {
  ["#assign-front-button", "#assign-middle-button", "#assign-back-button", "#auto-arrange-button", "#clear-arrangement-button", "#submit-arrangement-button"].forEach((selector) => {
    const el = $(selector);
...
function renderAssignedSection(sectionName, container) {
  ...
  cards.forEach((card) => {
    ...
    cardElement.addEventListener("click", () => {
      state.arrangement[sectionName] = state.arrangement[sectionName].filter((c) => c !== card);
      state.selectedCards.clear();
      renderArrangeSection();
    });
...
function renderHandCards() {
  ...
  const cards = sortCards(state.handData.hand.filter((card) => !assigned.has(card)));
  ...
  cards.forEach((card, index) => {
    ...
    cardElement.addEventListener("click", () => { placeCardToRow(card); });
    container.appendChild(cardElement);
  });
}
...
function renderResultBoards(results) {
  ...
  results.rankings.forEach((ranking, index) => {
    ...
    if (ranking.special) headText += ` • ✨ ${ranking.special.name}`;
...
function renderReadyArea() {
  ...
  players.forEach((player) => {
    ...
    if (rankInfo) {
      ...
      if (rankInfo.special) rankBadge += `<span class="royalty-badge">✨ ${rankInfo.special.name}</span>`;
...
function showAdminScreen() {
  if (!state.adminLoggedIn) {
    ...
    if (password !== ADMIN_PASSWORD) { alert("Wrong admin password."); return; }
...
setInterval(() => {
  ...
  if (timerEl) {
    ...
    if (showTableTimer) timerEl.textContent = Math.max(0, Math.ceil((state.room.phaseEndsAt - Date.now()) / 1000));
    else timerEl.textContent = "--";
  }
  if (zoomTimerEl) {
    zoomTimerEl.classList.toggle("hidden", !showZoomTimer);
    if (showZoomTimer) zoomTimerEl.textContent = Math.max(0, Math.ceil((state.room.phaseEndsAt - Date.now()) / 1000));
  }
  autoSubmitIfNeeded();
  hostController();
}, 700);
...
document.addEventListener("click", (event) => {
  ...
  if (tab) selectRow(tab.dataset.row);
...
if (adminSearchField && !adminSearchField.dataset.bound) {
  ...
  adminSearchField.addEventListener("input", (e) => { state.adminSearch = e.target.value; renderAdminUserList(); });
}
...
document.addEventListener("click", async (event) => {
  ...
  try {
    if (action === "watch") await spectateRoom(state.user, code, name);
    else await joinRoomByCode(state.user, code, name);
    await enterRoom(code);
  } catch (error) {
    console.error(error);
...
setInterval(() => {
  ...
  if (menu && !menu.classList.contains("hidden")) renderRoomList();
}, 30000);
...
if (sendMoneyBtn) {
  sendMoneyBtn.addEventListener("click", async () => {
    ...
    if (!to || !amount || amount <= 0) { if (errEl) errEl.textContent = "Enter a username and a positive amount."; return; }
    ...
    try {
      ...
      $("#send-amount").value = "";
      $("#send-note").value = "";
    } catch (error) { if (errEl) errEl.textContent = error.message || "Transfer failed."; }
  });
}
